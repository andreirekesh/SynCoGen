...
```bash
python generate_graph_data.py --vocab_dir <path> [options]
```
...
- **Type**: `str`
- **Required**: Yes
- **Description**: Path to vocabulary directory containing `building_blocks.json`, `reactions.json`, `compatibility.pt`, `fragment_features.pt`, and `meta.json` (generated by `preprocess.py`)
- **Example**: `--vocab_dir vocabulary/original`
...
- **Description**: Full path to save the output `.pt` file containing generated molecule graphs
- **Note**: Only used when `--save_graphs` is set. The parent directory will be created if it doesn't exist. Must have `.pt` extension.
- **Example**: `--output_path data/graphs/my_dataset.pt`
...
1. **Load Vocabulary**: Loads vocabulary from `--vocab_dir` directory (including `building_blocks.json`, `reactions.json`, `compatibility.pt`, etc.) and updates module-level constants
...
- **Output file** (specified by `--output_path`): List of PyTorch Geometric `Data` objects containing molecular graphs compatible with SynCoGen's training pipeline
...
python generate_graph_data.py \
  --vocab_dir <path_to>/vocabulary/original \
  --num_samples 1000 \
  --length 2 3 4 \
  --save_graphs \
  --output_path data/graphs/dataset_list_full.pt \
  --seed 42 \
  --sample_by_inverse_molwt
```
