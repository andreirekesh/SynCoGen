...
def to_dense_graph(
    ...
    nodes_onehot, node_padding_mask = to_dense_batch(x=x, batch=batch, max_num_nodes=max_num_nodes)
    ...
    edges_onehot[:, :, :, 0] = torch.triu(edges_onehot[:, :, :, 0]) + torch.triu(edges_onehot[:, :, :, 0], 1).transpose(-1, -2)
...
def to_dense_coords(
    ...
    if coordinates is not None and coords_mask is not None:
        coords, _ = to_dense_batch(x=coordinates, batch=batch, max_num_nodes=max_num_nodes)
        coords_padding_mask, _ = to_dense_batch(x=coords_mask, batch=batch, max_num_nodes=max_num_nodes)
...
def to_dense_bonds(
    ...
    if bonds is not None and bonds_len is not None:
        B = bonds_len.shape[0] if isinstance(bonds_len, torch.Tensor) else len(bonds_len)
        ...
        bonds_dense, bonds_padding_mask = to_dense_batch(x=bonds, batch=bonds_batch, max_num_nodes=max_num_bonds)
...
def to_dense_pharmacophores(
    ...
    if pharm_types is not None and pharm_len is not None:
        ...
        B = pharm_len.shape[0] if isinstance(pharm_len, torch.Tensor) else len(pharm_len)
        ...
        pharm_types_dense, pharm_padding_mask = to_dense_batch(x=pharm_types, batch=pharm_batch, max_num_nodes=max_num_pharm)
        pharm_pos_dense, _ = to_dense_batch(x=pharm_pos, batch=pharm_batch, max_num_nodes=max_num_pharm)
...
def encode_no_edge(edges_onehot: torch.Tensor) -> torch.Tensor:
    ...
    diag = torch.eye(edges_onehot.shape[1], dtype=torch.bool).unsqueeze(0).expand(edges_onehot.shape[0], -1, -1)
    edges_onehot[diag] = 0
    return edges_onehot
